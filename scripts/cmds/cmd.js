...
const { loadDirectory, validate } = require("../../bot/login/loadScripts");

const ROOT = path.resolve(__dirname, "../..");
const COMMANDS_DIR = path.join(ROOT, "scripts/cmds");
...
module.exports = {
	...
	onStart: async function ({ message, args, config, registry, event, setReactionHandler }) {
		...
		if (action === "loadall") {
			const ok = [];
			const fail = [];
			for (const dir of ["scripts/cmds", "scripts/events"]) {
		...
		if (action === "install" || action === "add") {
			...
			try {
				...
				return message.reply(
					...
					`from ${source || "code"} to ${isEvent ? "scripts/events" : "scripts/cmds"}/${fileName} and reloaded it live.`
				);
			}
			catch (error) {
